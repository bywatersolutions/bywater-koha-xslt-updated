...
<xsl:stylesheet version="1.0"
  ...
  exclude-result-prefixes="marc items str">
    ...
    <xsl:element name="img"><xsl:attribute name="class">materialtype mt_icon_<xsl:value-of select="$materialTypeCode"/></xsl:attribute><xsl:attribute name="src">/intranet-tmpl/prog/img/famfamfam/<xsl:value-of select="$materialTypeCode"/>.png</xsl:attribute><xsl:attribute name="alt"></xsl:attribute></xsl:element>
        ...
        <!--  old way of handling field 787 superseded by RT 45916
            ...
            <span class="results_summary description"><span class="label">Other Relationship: </span>
                <xsl:for-each select="marc:datafield[@tag=787]">
                    <xsl:call-template name="chopPunctuation">
                        <xsl:with-param name="chopString">
                            <xsl:call-template name="subfieldSelect">
                                <xsl:with-param name="codes">abcdghikmnorst</xsl:with-param>
                            </xsl:call-template>
                        </xsl:with-param>
                    </xsl:call-template>
                    <xsl:choose><xsl:when test="position()=last()"><xsl:text>.</xsl:text></xsl:when><xsl:otherwise><xsl:text>; </xsl:text></xsl:otherwise></xsl:choose>
...
>>>>>>> e57e15fcb4... BWS-XSLT - RT 45916 update MARC21slim2intranetDetail.xsl to comment out former handling of linking entry fields and use the new linkingEntryFields template instead
        </xsl:if>
